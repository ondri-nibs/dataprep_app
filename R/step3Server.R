#' @title step3Server
#'
#' @description Server logic for the step #3 (Visualize missingness by 
#' type) tab.
#' @author Jedid Ahn
#' 
...
step3Server <- function(input, output, session, rvDataDF, ONDRIMissingCodes){
  ...
  observeEvent(input$nextStep2, {
    ...
    if (ncol(dataDF) > 200){
      ...
      ONDRICasePlot <- ggplot2::ggplot(data = transDataDF1,
                                       ggplot2::aes(x = VARIABLE, y = SUBJECT, 
                                                    fill = VALUE)) +
        ggplot2::geom_raster() +
        ggplot2::scale_x_discrete(breaks = xScale, drop = FALSE) +
        ggplot2::scale_y_discrete(breaks = yScale, drop = FALSE) +
      ...
      ONDRICasePlot <- ggplot2::ggplot(data = transDataDF1,
        ...
        ggplot2::geom_vline(xintercept = 
                              seq(0.5, length(transDataDF1$VARIABLE) - 0.5, 
                                  1)) +
        ggplot2::scale_y_discrete(breaks = yScale, drop = FALSE) +
        ggplot2::scale_fill_manual(
          name = "Type",
